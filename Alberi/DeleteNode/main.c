...
int main(void) {
    ...
    //tree = TreeCreateRoot(&a[0], TreeCreateRoot(&a[1], TreeCreateRoot(&a[2], NULL, NULL), NULL), NULL);
    TreeWriteStdoutPreOrder(tree);
    TreeWriteStdoutInOrder(tree);
    TreeWriteStdoutPostOrder(tree);
    ElemType to_be_found = 21;
    tree = DeleteBstNode(tree, &to_be_found);
    TreeWriteStdoutPreOrder(tree);
    TreeWriteStdoutInOrder(tree);
    TreeWriteStdoutPostOrder(tree);
    TreeDelete(tree);
